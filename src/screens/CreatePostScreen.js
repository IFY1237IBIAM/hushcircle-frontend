...
// ── Mood SVG Icons (colors are hardcoded per mood, not themed) ────────────
...
const MessageIcon       = ({ size = 20, color }) => (
  <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
const HeartResourceIcon = ({ size = 20, color }) => (
  <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <Path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
  </Svg>
);
...
const LightbulbIcon = ({ size = 14, color }) => (
  <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <Path d="M9 21h6M12 3a6 6 0 0 1 6 6c0 2.22-1.21 4.16-3 5.2V17a1 1 0 0 1-1 1H10a1 1 0 0 1-1-1v-2.8C7.21 13.16 6 11.22 6 9a6 6 0 0 1 6-6z" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
  </Svg>
);

const ShieldIcon    = ({ size = 32, color }) => (
  <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
export default function CreatePostScreen({ navigation }) {
  ...
  const animateIn  = (anim) => Animated.spring(anim, { toValue: 1, useNativeDriver: true, tension: 60, friction: 8 }).start();
  const animateOut = (anim, cb) => Animated.timing(anim, { toValue: 0, duration: 200, useNativeDriver: true }).start(cb);
  ...
  const handlePost = async () => {
    ...
    await spinner.withSpinner(async () => {
      try {
        ...
        setContent(""); setMood("sadness"); setHashtags([]);
        if (res.data.crisisDetected) { setShowCrisisModal(true); animateIn(crisisAnim); }
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: C.bg }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      <ScrollView contentContainerStyle={{ padding: 20, paddingTop: 56, paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
        ...
        <View style={{ marginBottom: 28 }}>
          <Text style={{ fontSize: 32, color: C.text, fontFamily: "DMSerifDisplay_400Regular" }}>Share your heart</Text>
          <Text style={{ fontSize: 13, color: C.textMuted, fontFamily: "Nunito_400Regular", marginTop: 4 }}>Anonymous & safe. Always.</Text>
        </View>

        {/* Mood picker */}
        <Text style={{ color: C.accentSoft, fontFamily: "Nunito_500Medium", fontSize: 13, marginBottom: 12 }}>How are you feeling?</Text>
          ...
          {MOODS.map((m) => (
            <TouchableOpacity
              ...
              style={{
                ...
                borderColor: mood === m.key ? m.color : C.border,
                backgroundColor: mood === m.key ? m.color + "33" : C.card,
              }}
              onPress={() => setMood(m.key)}
            >
              <m.Icon size={18} color={mood === m.key ? m.color : C.textMuted} />
              <Text style={{ color: mood === m.key ? m.color : C.textMuted, fontFamily: "Nunito_500Medium", fontSize: 12 }}>
                {m.label}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        {/* Text input */}
        <Text style={{ color: C.accentSoft, fontFamily: "Nunito_500Medium", fontSize: 13, marginBottom: 12 }}>What's on your mind?</Text>
        <View style={{ backgroundColor: C.card, borderRadius: 16, borderWidth: 1, borderColor: selectedMood.color + "66", padding: 16, marginBottom: 16, minHeight: 160 }}>
          <TextInput
            style={{ color: C.text, fontFamily: "Nunito_400Regular", fontSize: 17, lineHeight: 28, flex: 1, minHeight: 120 }}
          ...
          />
          <Text style={{ color: content.length > 450 ? C.error : C.textMuted, fontFamily: "Nunito_400Regular", fontSize: 12, textAlign: "right", marginTop: 8 }}>
            {content.length}/500
          </Text>
        </View>
        ...
        {/* Hashtag preview */}
        {hashtags.length > 0 && (
          <View style={{ backgroundColor: C.card, borderRadius: 12, padding: 12, marginBottom: 10, borderWidth: 1, borderColor: C.accent + "44", flexDirection: "row", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
            <Text style={{ color: C.textMuted, fontFamily: "Nunito_500Medium", fontSize: 11 }}>{hashtags.length}/5 hashtags</Text>
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, flex: 1, justifyContent: "flex-end" }}>
              {hashtags.map((tag, i) => (
                <View key={i} style={{ backgroundColor: C.accent + "22", borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4, borderWidth: 1, borderColor: C.accent + "44" }}>
                  <Text style={{ color: C.accent, fontFamily: "Nunito_600SemiBold", fontSize: 12 }}>{tag}</Text>
                </View>
              ))}
            </View>
          </View>
        ...
        {hashtags.length >= 5 && (
          <View style={{ flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: "#D4A44C18", borderRadius: 10, padding: 10, marginBottom: 10, borderWidth: 1, borderColor: "#D4A44C44" }}>
            <AlertTriangleIcon size={13} color="#D4A44C" />
            <Text style={{ color: "#D4A44C", fontFamily: "Nunito_400Regular", fontSize: 12, flex: 1 }}>Max 5 hashtags — only the first 5 will be saved</Text>
          </View>
        )}

        {/* Privacy notices */}
        {[
          { Icon: LockIcon, text: "Your post is anonymous. Only your pseudonym is shown — never your real identity." },
          { Icon: BotIcon,  text: "Our AI monitors all posts to keep HushCircle safe. Harmful or bullying content will be automatically blocked." },
        ].map(({ Icon, text }, i) => (
          <View key={i} style={{ flexDirection: "row", alignItems: "flex-start", gap: 10, backgroundColor: C.card, borderRadius: 12, padding: 12, marginBottom: 12, borderWidth: 1, borderColor: C.border }}>
            <Icon size={14} color={C.accentSoft} />
            <Text style={{ color: C.textMuted, fontFamily: "Nunito_400Regular", fontSize: 12, lineHeight: 18, flex: 1 }}>{text}</Text>
          </View>
        ))}

        {/* Submit */}
        <TouchableOpacity
          style={{ borderRadius: 14, padding: 16, alignItems: "center", marginTop: 4, backgroundColor: selectedMood.color, opacity: !content.trim() ? 0.5 : 1 }}
          onPress={handlePost}
          disabled={!content.trim()}
        >
          <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
            <selectedMood.Icon size={18} color="#fff" />
            <Text style={{ color: "#fff", fontFamily: "Nunito_600SemiBold", fontSize: 16 }}>Share anonymously</Text>
      ...
      {/* Draft discard modal */}
      <DraftDiscardModal visible={showDiscardModal} mode="discard" onLeave={handleDiscardLeave} onCancel={() => setShowDiscardModal(false)} />
      ...
      <Modal visible={showCrisisModal} transparent animationType="none">
        <View style={{ flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: "rgba(0,0,0,0.75)", padding: 20 }}>
          <Animated.View style={[{ backgroundColor: C.card, borderRadius: 28, padding: 24, borderWidth: 1, borderColor: C.accent + "44", width: "100%", maxWidth: 380 }, modalTransform(crisisAnim)]}>
            <View style={{ width: 72, height: 72, borderRadius: 36, backgroundColor: C.accent + "22", justifyContent: "center", alignItems: "center", alignSelf: "center", marginBottom: 16 }}>
              <HeartResourceIcon size={36} color={C.accent} />
            </View>
            <Text style={{ color: C.text, fontFamily: "DMSerifDisplay_400Regular", fontSize: 28, textAlign: "center", marginBottom: 8 }}>You are not alone</Text>
            <Text style={{ color: C.accentSoft, fontFamily: "Nunito_500Medium", fontSize: 14, textAlign: "center", lineHeight: 22, marginBottom: 16 }}>
            ...
            <Text style={{ color: C.textMuted, fontFamily: "Nunito_400Regular", fontSize: 13, lineHeight: 22, textAlign: "center", marginBottom: 20 }}>
            ...
            <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 12 }}>
              <HeartResourceIcon size={14} color={C.accentSoft} />
              <Text style={{ color: C.text, fontFamily: "Nunito_600SemiBold", fontSize: 14 }}>Talk to someone now</Text>
            </View>
            {CRISIS_RESOURCES.map((resource, index) => {
              const Icon = ResourceIconMap[resource.icon];
              return (
                <TouchableOpacity key={index} style={{ flexDirection: "row", alignItems: "center", gap: 12, backgroundColor: C.inputBg, borderRadius: 14, padding: 12, marginBottom: 8, borderWidth: 1, borderColor: C.border }} onPress={() => resource.url && Linking.openURL(resource.url)} activeOpacity={resource.url ? 0.7 : 1}>
                  <View style={{ width: 40, height: 40, borderRadius: 12, backgroundColor: C.accent + "22", justifyContent: "center", alignItems: "center" }}>
                    <Icon size={20} color={C.accent} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={{ color: C.text, fontFamily: "Nunito_600SemiBold", fontSize: 13, marginBottom: 2 }}>{resource.name}</Text>
                    <Text style={{ color: C.textMuted, fontFamily: "Nunito_400Regular", fontSize: 12 }}>{resource.subtitle}</Text>
                  </View>
                  {resource.url && <ChevronRightIcon size={18} color={C.accent} />}
                </TouchableOpacity>
              );
            })}
            <TouchableOpacity style={{ backgroundColor: C.accent, borderRadius: 14, padding: 16, alignItems: "center", marginTop: 16 }} onPress={() => animateOut(crisisAnim, () => { setShowCrisisModal(false); navigation.navigate("Feed"); })}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                <Text style={{ color: "#fff", fontFamily: "Nunito_600SemiBold", fontSize: 15 }}>I'm okay, go to feed</Text>
                <ArrowRightIcon size={15} color="#fff" />
              </View>
            </TouchableOpacity>
            <Text style={{ color: C.textMuted, fontFamily: "Nunito_400Regular", fontSize: 12, textAlign: "center", marginTop: 16, lineHeight: 18 }}>
              You matter. This community is here for you.
            </Text>
          </Animated.View>
        </View>
      </Modal>
      ...
      <Modal visible={showBlockedModal} transparent animationType="none">
        <Pressable style={{ flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: "rgba(0,0,0,0.75)", padding: 20 }} onPress={() => animateOut(blockedAnim, () => setShowBlockedModal(false))}>
          <Animated.View style={[{ backgroundColor: C.card, borderRadius: 28, padding: 24, borderWidth: 1, borderColor: C.border, width: "100%", maxWidth: 360 }, modalTransform(blockedAnim)]} onStartShouldSetResponder={() => true}>
            <View style={{ width: 72, height: 72, borderRadius: 36, backgroundColor: blocked.color + "22", justifyContent: "center", alignItems: "center", alignSelf: "center", marginBottom: 16 }}>
              <BlockedIcon size={32} color={blocked.color} />
            </View>
            <Text style={{ color: C.text, fontFamily: "DMSerifDisplay_400Regular", fontSize: 24, textAlign: "center", marginBottom: 10 }}>{blocked.title}</Text>
            <Text style={{ color: C.textMuted, fontFamily: "Nunito_400Regular", fontSize: 13, textAlign: "center", lineHeight: 22, marginBottom: 16 }}>{blocked.message}</Text>
            <View style={{ backgroundColor: C.inputBg, borderRadius: 12, padding: 14, borderWidth: 1, borderColor: blocked.color + "44", marginBottom: 20 }}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 5, marginBottom: 4 }}>
                <LightbulbIcon size={13} color={C.accentSoft} />
                <Text style={{ color: C.accentSoft, fontFamily: "Nunito_600SemiBold", fontSize: 12 }}>Tip</Text>
              </View>
              <Text style={{ color: C.textMuted, fontFamily: "Nunito_400Regular", fontSize: 13, lineHeight: 20 }}>{blocked.tip}</Text>
            </View>
            <View style={{ flexDirection: "row", gap: 10 }}>
              <TouchableOpacity style={{ flex: 1, padding: 14, borderRadius: 14, borderWidth: 1, borderColor: C.border, alignItems: "center" }} onPress={() => animateOut(blockedAnim, () => setShowBlockedModal(false))}>
                <Text style={{ color: C.text, fontFamily: "Nunito_500Medium", fontSize: 14 }}>Edit my post</Text>
              </TouchableOpacity>
              <TouchableOpacity style={{ flex: 1, padding: 14, borderRadius: 14, alignItems: "center", backgroundColor: blocked.color }} onPress={() => animateOut(blockedAnim, () => setShowBlockedModal(false))}>
                <Text style={{ color: "#fff", fontFamily: "Nunito_600SemiBold", fontSize: 14 }}>Understood</Text>
              </TouchableOpacity>
            </View>
          </Animated.View>
        </Pressable>
      ...
      <NoNetworkOverlay visible={showNoNetwork} action="post" onClose={() => setShowNoNetwork(false)} onRetry={() => { setShowNoNetwork(false); handlePost(); }} />
