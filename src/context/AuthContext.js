/**
 * AuthContext.js — Production (True WebAuthn passkeys)
 *
 * Changes from previous version:
 *   - login() returns twoStep flag for two-step PIN screen
 *   - loginWithPasskey() removed (WebAuthn is now handled by the hook + server)
 *   - setAuth() is exposed so AuthScreen can call it after PIN verify or passkey auth
 */

import React, { createContext, useState, useContext, useEffect, useCallback } from "react";
import AsyncStorage from "@react-native-async-storage/async-storage";
import api           from "../api/api";
import { clearVibeCache } from "../components/AvatarVibeEmoji";
...
export const AuthProvider = ({ children }) => {
  ...
  // ── Token refresh (bypasses axios interceptor) ───────────────────────────────
  ...
  const loadStoredAuth = async () => {
    try {
      ...
      if (!activeAcc) {
        ...
        setUser({ pseudonym: activeAcc.pseudonym, email: activeAcc.email });
      }
      ...
      try {
        ...
        if (refreshError.status === 401) await clearAuth();
      }
    } catch (error) {
  ...
  const setAuth = async (newToken, newUser) => {
    ...
    if (exists) {
      updated = savedAccounts.map((a) =>
        a.pseudonym === newUser.pseudonym ? { ...a, token: newToken, email: newUser.email } : a
      );
    } else {
      updated = [
        ...savedAccounts,
        { pseudonym: newUser.pseudonym, email: newUser.email, token: newToken, avatarColor: avatarColorForPseudonym(newUser.pseudonym) },
      ];
    }
    await saveAccountsToStorage(updated);
  ...
  // ── Login ─────────────────────────────────────────────────────────────────────
  // Returns:
  //   { unverified: true }               → email not verified
  //   { twoStep: true, pendingToken, pendingUser, twoStepHint }  → PIN required
  //   normal response data               → fully logged in
  ...
  const login = async (email, password, deviceInfo = {}) => {
    ...
    const { token: newToken, user: newUser, twoStep, twoStepHint } = response.data;

    if (newUser.isVerified === false) return { user: newUser, unverified: true };

    if (twoStep) {
      return { twoStep: true, twoStepHint: twoStepHint || "", pendingToken: newToken, pendingUser: newUser };
    }
  ...
  const addAccount = async (email, password) => {
    ...
    if (newUser.isVerified === false) return { success: false, message: "This account hasn't been verified yet." };
    ...
    if (alreadyExists) {
      ...
      await saveAccountsToStorage(updated); setAccounts(updated);
      await _activateAccount(newToken, newUser);
      return { success: true, alreadyExisted: true };
    }
    const newEntry = { pseudonym: newUser.pseudonym, email: newUser.email, token: newToken, avatarColor: avatarColorForPseudonym(newUser.pseudonym) };
  ...
  const switchAccount = useCallback(async (pseudonym, overrideToken, overrideUser) => {
    clearVibeCache(); // ← wipe emoji cache on account switch
    const savedAccounts = await loadAccountsFromStorage();
  ...
  const removeAccount = async (pseudonym) => {
    if (user?.pseudonym === pseudonym) throw new Error("Cannot remove the active account. Switch first.");
    const savedAccounts = await loadAccountsFromStorage();
  ...
  const clearReinstatedStatus = async () => {
    try { await api.patch("/auth/clear-reinstated"); } catch {}
  ...
  const logout = async () => {
    ...
    clearVibeCache(); // ← wipe emoji cache on logout
    const savedAccounts   = await loadAccountsFromStorage();
    const activePseudonym = user?.pseudonym;
    const remaining       = savedAccounts.filter((a) => a.pseudonym !== activePseudonym);
    await saveAccountsToStorage(remaining); setAccounts(remaining);
    if (remaining.length > 0) {
      try { await switchAccount(remaining[0].pseudonym); return; } catch {}
    }
    await AsyncStorage.multiRemove(["token", "user", ACTIVE_KEY]);
  ...
  const refreshUser = async () => {
    ...
    try {
      ...
      setUser(freshUser);
      await AsyncStorage.setItem("user", JSON.stringify(freshUser));
    } catch (e) { console.log("Refresh user error:", e.message); }
  ...
  return (
    <AuthContext.Provider value={{
      user, token, loading,
      signup, login, logout,
      updateUser, clearReinstatedStatus, refreshUser,
      setAuth,
      accounts, addAccount, switchAccount, removeAccount, getAllAccounts,
    }}>
      {children}
